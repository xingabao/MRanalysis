    ...
    wellPanel(
      ...
      column(width = 4,
             ...
             tags$div(id = 'variable-des',  
                      ...
                      HTML("<span class='help-block m-b-none var-select'><i class='fa fa-info-circle'></i> [required] <span class='var-var'>geneID</span>, <span class='var-num'>one</span>. The geneID column represents the list of genes that are associated with each enriched term.</span>"),
...
server.gallery <- function(input, output, session) {
  ...
  output[['ui_aesthetics']] <- renderUI({
    ...
    aesthetics <- c('Description', 'ONTOLOGY', 'pval', 'geneID', 'Count')
    data = imported$data()
    if (!is.null(data)) {
  ...
  display.status <- reactive({
    !is.null(input[["dragvars"]]$target$Description) &&
      ...
      !is.null(input[["dragvars"]]$target$geneID) &&
  ...
  output[['is_add_plot_buttom']] <- renderUI({
    if (display.status()) {
      tags$div(
        ...
        fluidRow(column(width = 6, sliderInput(inputId = "WIDTH", label = h5("Width"), value = 1200,  min = 100, max = 3000, step = 10)),
  ...
  db.gene_symbol <- debounce(reactive({ input[['gene_symbol']] * 9 }), DEBOUNCE.0)
  db.count <- debounce(reactive({ input[['count']] * 9 }), DEBOUNCE.0)
  db.go_ontology <- debounce(reactive({ input[['go_ontology']] * 32 }), DEBOUNCE.0)
  db.newline <- debounce(reactive({ input[['newline']] }), DEBOUNCE.0)
  ...
  plot.reactive <- function() {
    ...
    # GO <- GO[, c('Description', 'ONTOLOGY', 'p.adjust', 'Count', 'geneID')]
    GO <- i.data()
    colnames(GO) <- c('Description', 'ONTOLOGY', 'p.adjust', 'geneID', 'Count')
    ...
    for (i in 1:nrow(datasig)) {
      ...
      nr <- ceiling(current_row$Count / newline)
      if (current_row$Count > newline) {
        new_row <- as.data.frame(matrix(NA, nrow = nr, ncol = length(columns)))
        new_row[, which(columns == 'ONTOLOGY')] <- rep(current_row$ONTOLOGY, nr)
        new_row[, which(columns == '-log10(P)')] <- rep(0, nr)
      } else {
        new_row <- as.data.frame(matrix(NA, nrow = 1, ncol = length(columns)))
        new_row[, which(columns == 'ONTOLOGY')] <- rep(current_row$ONTOLOGY, 1)
        new_row[, which(columns == '-log10(P)')] <- rep(0, 1)
      }
      colnames(new_row) <- columns
      GO. <- rbind(GO., current_row, new_row)
    }
    replace_fifth_slash <- function(text) {
      parts <- str_split(text, "/")[[1]]
      if (length(parts) > newline) {
        parts[newline + 1] <- paste0("\n", parts[newline + 1])
      }
      return(paste(parts, collapse = "/"))
    }
    
    GO. <- GO. %>% mutate(geneID = sapply(geneID, replace_fifth_slash))
    ...
    GO. <- GO. %>% 
      ...
      mutate(Description = if_else(is.na(Description), glue("<span style='color:#00000000'>{id}</span>"), glue("<span style='color:{Color}'>{Description}</span>"))) %>% 
      mutate(Description = factor(Description, levels = rev(Description)))
    
    GO.$id <- factor(GO.$id, levels = GO.$id %>% rev())
    GO.$ONTOLOGY <- factor(GO.$ONTOLOGY)
    ...
    # Draw plot
    gg <- ggplot(GO., aes(`-log10(P)`, Description)) +
      geom_bar(aes(fill = ONTOLOGY), stat = "identity", show.legend = FALSE) +
      scale_fill_manual(values = palette.map) +
      geom_text(data = GO. %>% drop_na(), aes(label = Count), size = db.count(), hjust = -0.25, vjust = 0.15, colour = input[['count_col']]) + 
      geom_text(
        data = GO. %>% drop_na() %>% filter(Count <= newline),
        aes(x = 0.01, label = geneID),
        vjust = 0.5, hjust = 0, size = db.gene_symbol(), nudge_y = -1, show.legend = FALSE, fontface = "italic",
      ) +
      geom_text(
        data = GO. %>% drop_na() %>% filter(Count > newline),
        aes(x = 0.01, label = geneID),
        vjust = 0.5, hjust = 0, size = db.gene_symbol(), nudge_y = -1.5, show.legend = FALSE, lineheight = 1., fontface = "italic",
      ) +
      facet_grid(
        ONTOLOGY ~ .,
        scales = "free_y",
      ...
      theme(
        ...
        strip.text.y = element_markdown(angle = 0, size = db.go_ontology(), face = "bold"),
        strip.background = element_rect(fill = "#EEEEEE", color = "transparent"), 
        axis.ticks.y = element_blank(),
        axis.text.y = element_markdown(size = db.axis_size()),
        axis.text.x = element_text(face = "bold", angle = 0, hjust = 1, vjust = 1, size = db.axis_size()),
    ...
    for (i in stripr) {
      ...
      hh$grobs[[i]]$grobs[[1]]$children[[j]]$gp$fill <- paste0(substr(palette.map[k], 1, 7), '7F')
      k <- k+1
    }
  ...
  observeEvent(input[['show_data-confirm']], {
    ...
    observeEvent(input[['plot_button']], {
      ...
      req(db.gene_symbol(), cancelOutput = TRUE)
      req(db.count(), cancelOutput = TRUE)
      req(db.go_ontology(), cancelOutput = TRUE)
      req(db.newline(), cancelOutput = TRUE)
  ...
  observeEvent(input[['show_sessioninfo']], {
    displayCodeModal(
      code = as.character(xfun::session_info(c('openxlsx', 'dplyr', 'tidyr', 'glue', 'grid', 'stringr', 'ggplot2', 'ggtext'))),
  ...
  help.modal <- modalDialog(
    ...
    tags$p(style = 'text-align:left', tags$strong("Gene Involvement (Gene labels in the bars)"), ': Inside each bar, you see the names of the genes that are associated with the specific GO term.'),
