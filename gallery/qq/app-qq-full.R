...
ui.gallery <- function() {
  ...
  www = 'www/data'
  qq_demo <<- data.table::fread(glue('{www}/demo.csv'))
...
server.gallery <- function(input, output, session) {
  ...
  # Import Data
  imported <- import_server_demo(id = "show_data", choices = c('qq_demo'), return_class = "tbl_df", read_fns = list(tsv = function(file) { data.table::fread(fread) }, gz = function(file) { data.table::fread(file) } ))
