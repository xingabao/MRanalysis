    ...
    wellPanel(
      ...
      column(width = 4,
             ...
             tags$div(id = 'variable-des',  
                      HTML("<span class='help-block m-b-none var-select'><i class='fa fa-info-circle'></i> [required] <span class='var-var'>ID</span>, <span class='var-num'>one</span>. The unique identifier for the KEGG term.</span>"),
                      HTML("<span class='help-block m-b-none var-select'><i class='fa fa-info-circle'></i> [required] <span class='var-var'>Description</span>, <span class='var-num'>one</span>. A brief description of the KEGG term.</span>"),
                      HTML("<span class='help-block m-b-none var-select'><i class='fa fa-info-circle'></i> [required] <span class='var-var'>BgRatio</span>, <span class='var-num'>one</span>. The ratio of genes associated with the KEGG term in the background set (e.g., all genes).</span>"),
                      HTML("<span class='help-block m-b-none var-select'><i class='fa fa-info-circle'></i> [required] <span class='var-var'>GeneRatio</span>, <span class='var-num'>one</span>. The ratio of genes associated with the KEGG term in your input list.</span>"),
...
server.gallery <- function(input, output, session) {
  ...
  output[['download_demo_data']] <- downloadHandler(
    filename <- function(){ paste('KEGG.xlsx') },
    content <- function(file){
      file.copy(glue("{www}/KEGG.xlsx"), file)
    }, contentType = NULL)
  ...
  output[['ui_aesthetics']] <- renderUI({
    ...
    aesthetics <- c('ID', 'Description', 'BgRatio', 'GeneRatio', 'pval')
    data = imported$data()
    if (!is.null(data)) {
  ...
  plot.reactive <- function() {
    ...
    # 加载数据集
    # KEGG <- openxlsx::read.xlsx(xlsxFile = 'E:/BaiduNetdiskWorkspace/005.Bioinformatics/MRanalysis/gallery/kegg-circos-plot/www/data/KEGG.xlsx')
    # ------------------------------------------------------------------------------------------------------------
    KEGG <- i.data()
    colnames(KEGG) <- c('ID', 'Description', 'BgRatio', 'GeneRatio', 'p.adjust')
    ...
    if (input[['self_palette']]) {
      ontology.col <- c.palette(input[['palette_outer']])[1]
      logpvalue.col <- c.palette(input[['palette_inner']])
    } else {
      ontology.col <- input[['term_col']]
      logpvalue.col <- c(input[['low_col']], input[['high_col']])
    }
    
    # 数据整理
    # ------------------------------------------------------------------------------------------------------------
    KEGG$Description <- str_wrap(KEGG$Description, 40)
    KEGG$ONTOLOGY <- "KEGG"
    data <- KEGG[order(KEGG$p.adjust), ]
    datasig <- data[data$p.adjust < pvalueFilter,, drop = FALSE]
    KEGG <- datasig[datasig$ONTOLOGY == "KEGG",, drop = FALSE]
    data <- na.omit(head(KEGG, shownum))
    ...
    bed4 <-  data.frame(KEGG = data$ID, start = 1, end = max(BgGene), ratio = ratio, col = ontology.col)
    ...
    }, track.height = 0.08, bg.border = NA, bg.col = ontology.col)
    
    for(si in get.all.sector.index()) {
      circos.axis(
        h = "top", labels.cex = db.axis_size(), sector.index = si,track.index = 1,
    ...
    KEGG <- KEGG[1:shownum, ]
    KEGG.legend <- Legend(
      labels = paste0(KEGG$ID, ':', KEGG$Description),
      type = "points", pch = 15,
      title_gp = gpar(fontsize = db.legend_size()),
      labels_gp = gpar(fontsize = db.legend_size()),
      legend_gp = gpar(col = ontology.col), title_position = "topleft",
      title = "KEGG", nrow = nrow(KEGG), size = unit(3, "mm"), grid_height = unit(5, "mm"),
    ...
    lgd <- packLegend(KEGG.legend, middle.legend, logp.legend)
  ...
  output[['workflow_code']] <- metaRender(renderText, {
    ...
    # data <- openxlsx::read.xlsx(xlsxFile = 'E:/BaiduNetdiskWorkspace/005.Bioinformatics/MRanalysis/gallery/kegg-circos-plot/www/data/KEGG.xlsx')
    ...
    KEGG <- ..(if ((class(imported$code()) == 'name')[1]) { quote(data %>% ungroup()) } else { imported$code() } ) %>%
      dplyr::select(..(input[['dragvars']]$target$ID), ..(input[['dragvars']]$target$Description), ..(input[['dragvars']]$target$BgRatio), ..(input[['dragvars']]$target$GeneRatio), ..(input[['dragvars']]$target$pval))
    '# Set palette'
    ontology.col <- ..(
      if (input[['self_palette']]) {
        c.palette(input[['palette_outer']])[1]
      } else {
        input[['term_col']]
      }
    )
    logpvalue.col <- ..(
      if (input[['self_palette']]) {
        c.palette(input[['palette_inner']])[1:2]
      } else {
    ...
    '# Draw plot'
    pdf(file = 'KEGG-circis.pdf', height = ..(db.height()/100), width = ..(db.width()/100))
    par(omi = c(0.1, 0.1, 0.1, ..(db.margin_right())))
    circos.par(track.margin = c(0.01, 0.01))
    circos.genomicInitialize(df, plotType = "none")
    circos.trackPlotRegion(ylim = c(0, 1), panel.fun = function(x, y) {
