...
server.gallery <- function(input, output, session) {
  ...
  plot.reactive <- function() {
    ...
    KEGG <- KEGG %>% dplyr::select(Description, Count, level2, Color) %>%
      mutate(rank = 1) %>%
      group_by(level2) %>%
      reframe(Description = NA, Count = 0, Color = NA, rank = 0) %>%
      bind_rows( ., KEGG) %>%
      arrange(level2, rank) %>%
      mutate(
        Description = as.character(Description), 
        Description = case_when(
          is.na(Description) ~ glue("<strong style='color:#000000;font-size:{db.strip_text()}px'>{level2}</strong>"),
          TRUE ~ Description
        )
      ) %>%
      mutate(Description = factor(Description, levels = rev(Description)))
    
    max_count = max(KEGG$`-log10(P)`, na.rm = TRUE)
    ...
    gg <- ggplot(KEGG, aes(x = `-log10(P)`, y = Description)) +
      ...
      geom_text(aes(label = ifelse(Count == 0, '', Count)), size = db.count(), hjust = -0.25, vjust = 0.15, colour = input[['count_col']]) +
      labs(
      ...
      theme(
        ...
        plot.title = element_text(hjust = -2, size = 15, face = "bold"), 
  ...
  output[['workflow_code']] <- metaRender(renderText, {
    ...
    data <- openxlsx::read.xlsx(xlsxFile = 'E:/BaiduNetdiskWorkspace/005.Bioinformatics/MRanalysis/gallery/kegg-bar-plot2-hierarchy/www/data/KEGG.xlsx')
    ...
    hierarchy <- openxlsx::read.xlsx(xlsxFile = 'E:/BaiduNetdiskWorkspace/005.Bioinformatics/MRanalysis/gallery/kegg-bar-plot2-hierarchy/www/data/kegg_hierarchy.xlsx')
    ...
    KEGG <- KEGG %>% dplyr::select(Description, Count, level2, Color) %>%
      ...
      mutate(
        Description = as.character(Description), 
        Description = case_when(
          is.na(Description) ~ paste0("<strong style='color:#000000;font-size:", ..(db.strip_text()), "px'>", level2, "</strong>"),
          TRUE ~ Description
        )
      ) %>%
      mutate(Description = factor(Description, levels = rev(Description)))
  ...
  help.modal <- modalDialog(
    ...
    tags$p(style = 'text-align:left', "This KEGG enrichment bar plot visualizes the significantly enriched pathways based on a set of genes from an experiment (e.g., gene expression or genome-wide association study). Each bar represents a specific KEGG pathway, showing the number of genes involved, as well as how the pathways are grouped into broader biological categories."),
    tags$hr(),
    ...
    tags$p(style = 'text-align:left', tags$strong("Y-axis (Pathways)"), ': 1) The Y-axis lists the enriched KEGG pathways. Each pathway corresponds to a specific biological process or disease. 2) The pathways are grouped into broader biological categories (e.g., "Cardiovascular disease", "Endocrine and metabolic disease", "Immune system", etc.). These categories help in interpreting the biological context of the results.'),
    tags$p(style = 'text-align:left', tags$strong("Bar Length (Enrichment Significance) (X-axis)"), ': The length of each bar corresponds to the -log10(p-value), which indicates the statistical significance of the enrichment. A longer bar represents a more significant result.'),
    tags$p(style = 'text-align:left', tags$strong("Gene Counts (Numbers on Bars)"), ": The small numbers at the end of each bar represent the number of genes associated with the respective KEGG term."),
    tags$p(style = 'text-align:left', tags$strong("Categories (Black Color)"), ': The pathways are grouped into higher-level biological categories, which are shown on the X-axis in black color. For example, pathways related to "Immune disease" or "Infectious disease: bacterial" are grouped together. These categories help in interpreting the broader biological context of the enriched pathways.'),
