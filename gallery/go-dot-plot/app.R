...
server.gallery <- function(input, output, session) {
  ...
  output[['ui_aesthetics']] <- renderUI({
    ...
    aesthetics <- c('Description', 'ONTOLOGY', 'BgRatio', 'GeneRatio', 'pval', 'Count')
    data = imported$data()
    if (!is.null(data)) {
  ...
  output[['is_add_plot_buttom']] <- renderUI({
    if (display.status()) {
      tags$div(
        ...
        fluidRow(column(width = 6, sliderInput(inputId = "WIDTH", label = h5("Width"), value = 880,  min = 100, max = 3000, step = 10)),
  ...
  # DEBOUNCE
  db.axis_title <- debounce(reactive({ input[['axis_title']] * 10 + 10 }), DEBOUNCE.0)
  db.axis_size <- debounce(reactive({ input[['axis_size']] * 10 + 10 }), DEBOUNCE.0)
  db.legend_title <- debounce(reactive({ input[['legend_title']] * 10 + 10 }), DEBOUNCE.0)
  db.legend_text <- debounce(reactive({ input[['legend_text']] * 10 + 10 }), DEBOUNCE.0)
  db.strip_size <- debounce(reactive({ input[['strip_size']] * 10 + 10 }), DEBOUNCE.0)
  ...
  plot.reactive <- function() {
    ...
    # 加载数据集
    # GO <- openxlsx::read.xlsx(xlsxFile = 'E:/BaiduSyncdisk/005.Bioinformatics/MRanalysis/gallery/go-dot-plot/www/data/GO.xlsx')
    # ------------------------------------------------------------------------------------------------------------
    GO <- i.data()
    colnames(GO) <- c('Description', 'ONTOLOGY', 'BgRatio', 'GeneRatio', 'p.adjust', 'Count')
    ...
    if (input[['self_palette']]) {
      logpvalue.col <- c.palette(input[['palette']])
    } else {
    ...
    data <- data[data$p.adjust < pvalueFilter,, drop = FALSE]
    BP <- data[data$ONTOLOGY == "BP",, drop = FALSE]
    CC <- data[data$ONTOLOGY == "CC",, drop = FALSE]
    MF <- data[data$ONTOLOGY == "MF",, drop = FALSE]
    ...
    # Draw plot
    # ------------------------------------------------------------------------------------------------------------
    eGo <- separate(data = datasig, col = GeneRatio,into = c("GR1", "GR2"), sep = "/")
    ...
    # Draw plot
    gg <- ggplot(eGo, aes(enrichment_factor, Description))
    gg <- ggplot(eGo, aes(enrichment_factor, fct_reorder(factor(Description), enrichment_factor)))
    gg <- gg + geom_point(aes(size = Count, color = -1*log10(p.adjust), shape = ONTOLOGY)) +
      scale_color_gradient(low = logpvalue.col[1], high = logpvalue.col[2]) + 
      labs(
        color = expression(-log[10](P)), size = "Count", shape = "Ontology",
        x = "Enrichment Factor",
        y = "GO term"
      ) + 
      theme_bw() +
      theme(
        axis.title = element_text(size = db.axis_title(), color = '#000000'),
        axis.text = element_text(size = db.axis_size(), color = '#000000'),
        legend.title = element_text(size = db.legend_title(), color = '#000000'),
        legend.text = element_text(size = db.legend_text(), color = '#000000'),
        strip.text = element_text(size = db.strip_size(), color = '#000000')
      )
    
    message(glue('{Sys.time()} Plot Success'))
    if (input[['layout_type']] == 'A') {
      gg
    } else if (input[['layout_type']] == 'B') {
      gg + facet_wrap( ~ ONTOLOGY)
    } else {
      gg + facet_wrap( ~ ONTOLOGY, ncol = 1, scale = 'free')
    }
  }
  ...
  observeEvent(input[['show_data-confirm']], {
    ...
    observeEvent(input[['plot_button']], {
      ...
      req(db.legend_title(), cancelOutput = TRUE)
      req(db.legend_text(), cancelOutput = TRUE)
      req(db.strip_size(), cancelOutput = TRUE)
  ...
  output[['workflow_code']] <- metaRender(renderText, {
    ...
    GO <- ..(if ((class(imported$code()) == 'name')[1]) { quote(data %>% ungroup()) } else { imported$code() } ) %>% 
      dplyr::select(..(input[['dragvars']]$target$Description), ..(input[['dragvars']]$target$ONTOLOGY), ..(input[['dragvars']]$target$BgRatio), ..(input[['dragvars']]$target$GeneRatio), ..(input[['dragvars']]$target$pval), ..(input[['dragvars']]$target$Count))
    
    '# Set palette'
    logpvalue.col <- ..(if (input[['self_palette']]) {
      c.palette(input[['palette']])
    } else {
      c(input[['low_col']], input[['high_col']])
    })
  ...
  observeEvent(input[['show_sessioninfo']], {
    displayCodeModal(
      code = as.character(xfun::session_info(c('openxlsx', 'dplyr', 'tidyr', 'stringr', 'ggplot2', 'forcats'))),
  ...
  help.modal <- modalDialog(
    ...
    tags$hr(),
    tags$h3('What is a GO dot plot?'),
    tags$p(style = 'text-align:left', "A GO dot plot is a visual representation used in bioinformatics to display the results of Gene Ontology (GO) enrichment analysis. It helps in understanding the biological significance of gene sets."),
    tags$p(style = 'text-align:left', "The GO dot plot helps researchers quickly identify which GO terms are significantly enriched in a given dataset, providing insights into the biological processes, molecular functions, or cellular components that are overrepresented."),
    tags$p(style = 'text-align:left', "Here’s what it typically involves:"),
    tags$h4('Key Features'),
    tags$hr(),
    tags$p(style = 'text-align:left', tags$strong("GO Terms"), ": Each point represents a different GO term related to biological processes, molecular functions, or cellular components."),
    tags$p(style = 'text-align:left', tags$strong("-log10(p-value)"), ": Plotted on the x-axis to show the significance of enrichment. A higher value indicates greater significance."),
    tags$p(style = 'text-align:left', tags$strong("Dot Size"), ": Corresponds to the number of genes associated with each GO term."),
    tags$p(style = 'text-align:left', tags$strong("Dot Color"), ": Often used to represent another variable, such as the p-value or fold enrichment."),
