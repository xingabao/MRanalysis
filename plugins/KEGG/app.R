...
ui.alb <- function() {
  ...
  tags$head(list(web.statistic.baidu, web.statistic.google))
  ...
  bs4Dash::dashboardPage(
    ...
    sidebar = bs4DashSidebar(
      ...
      bs4SidebarMenu(
        ...
        bs4SidebarMenuItem(HTML('<p class="nav-text">KEGG enrichment Analysis</p>'), tabName = 'tab-ana', icon = icon('calculator',lib = 'font-awesome', class = 'nav-icon')),
    ...
    body = bs4Dash::dashboardBody(
      ...
      bs4TabItems(
        ...
        bs4Dash::bs4TabItem(
          ...
          br(),
          {
            html. <- system.file('extdata', 'HTML/intro.plugins.KEGG.html', package = 'MRanalysisBase')
            tryCatch(
              expr = {
                if (nzchar(html.) && file.exists(html.)) {
                  suppressWarnings(shiny::includeHTML(path = html.))
        ...
        bs4TabItem(
          ...
          fluidRow(
            ...
            bs4Dash::box(
              id = 'step_para_one',
              title = HTML("<strong><i class='fa-solid fa-play'></i>&nbsp;&nbsp;&nbsp;Step 2. Parameters for KEGG Enrichment Analysis.</strong>"),
              status = 'warning',
              width = 4,
              collapsed = FALSE,
              collapsible = TRUE,
  ...
  observeEvent(input[['confirm_analysis']], {
    ...
    shiny::insertUI(
      ...
      ui =  bs4Dash::tabBox(
        ...
        tabPanel(
          title = 'Bar Plot',
          fluidRow(
            column(width = 3, shiny::sliderInput(inputId = 'bar_plot_top', label = h6("Show Top Items"), min = 1, max = 20, value = 10, step = 1)),
        ...
        tabPanel(
          title = 'Dot Plot',
          fluidRow(
            column(width = 3, shiny::sliderInput(inputId = 'dot_plot_top', label = h6("Show Top Items"), min = 1, max = 20, value = 10, step = 1)),
    ...
    shiny::insertUI(
      ...
      ui =  bs4Dash::box(
        ...
        title = HTML('KEGG Enrichment Result'),
        status = 'success',
        width = 12,
        background = NULL,
        withSpinner({ DT::dataTableOutput(outputId = paste0('enrichment_data', enrichment_data_random())) }, type = 8, color = '#EE3F4D', size = 1, color.background = '#2F2F35', caption = 'Performing KEGG Enrichment Analysis, please be patient...'))
    )
    ...
    output[[paste0('enrichment_data', enrichment_data_random())]]  <- DT::renderDT({
      spsComps::shinyCatch({
        ...
        for (index in 1:nrow(kegg@result)) {
          SS = c()
          gid = kegg@result[index, 'geneID']
          gids = str_split(gid, '/')[[1]]
          for (g in gids) {
            S = hg[hg$ENTREZID == g, ]$SYMBOL
            SS = c(SS, S)
          }
          kegg@result[index, 'geneSymbol'] = paste0(SS, collapse = '/')
        }
        kegg@result['ONTOLOGY'] = 'KEGG'
        kegg.table(kegg)
      })
      
      if (!is.null(kegg.table())) {
        ...
        as.datatable(
          ...
          options = list(
            ...
            paging = TRUE, 
            buttons = list(
              list(extend = 'copy', filename =  'KEGG_enrichment', title = 'KEGG_enrichment', exportOptions = list(modifier = list(page = 'current'))),
              list(extend = 'print', filename =  'KEGG_enrichment', title = 'KEGG_enrichment', exportOptions = list(modifier = list(page = 'current'))),
              list(extend = 'collection',
                   buttons = list(
                     list(
                       extend = 'csv', 
                       filename = 'KEGG_enrichment',
    ...
    output[[paste0('bar_plot', enrichment_data_random())]] <- renderUI({
      req(kegg.table(), cancelOutput = TRUE)
      fluidRow(
